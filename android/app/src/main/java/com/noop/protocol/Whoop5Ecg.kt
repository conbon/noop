...
object Whoop5Ecg {
    ...
    /** Human-readable name of a Labrador opcode, for logs and the probe report. */
    fun commandName(cmd: Int): String = when (cmd) {
        SELECT_WRIST_CMD -> "SELECT_WRIST"
        MAIN_CONTROL_ECG_DATA_GENERATION_CMD -> "TOGGLE_LABRADOR_DATA_GENERATION"
        TOGGLE_SAVE_RAW_ECG_CMD -> "TOGGLE_LABRADOR_RAW_SAVE"
        TOGGLE_REALTIME_FILTERED_ECG_CMD -> "TOGGLE_LABRADOR_FILTERED"
        else -> "cmd$cmd"
    }

    /** True for the four Labrador opcodes this file knows. */
    fun isLabradorCommand(cmd: Int): Boolean =
        cmd == SELECT_WRIST_CMD || cmd == MAIN_CONTROL_ECG_DATA_GENERATION_CMD ||
            cmd == TOGGLE_SAVE_RAW_ECG_CMD || cmd == TOGGLE_REALTIME_FILTERED_ECG_CMD
    ...
    fun innerPayload(frame: ByteArray, payloadStart: Int = PUFFIN_PAYLOAD_START): List<Int>? {
        ...
        if (Crc.crc16Modbus(frame.copyOfRange(0, 6)) != gotHeader) return null
        ...
        if (Crc.crc32(frame.copyOfRange(8, payloadEnd)) != gotCrc32) return null
